...
class Employee
{
    ...
    function __construct(int $employee_id, string $first_name, string $last_name, string $location)
    {
...
class Employees
{
    ...
    public function add(Employee $employee): bool
    {
        ...
        $stmt = $this->connection->prepare("INSERT INTO `employees` (id, first_name, last_name, location) VALUES (?, ?, ?, ?)");
    ...
    public function get(): array
    {
        ...
        while ($row = $result->fetch_assoc()) {
            $employee = new Employee(intval($row["id"]), $row["first_name"], $row["last_name"], $row["location"]);
            $employees[] = $employee;
        }
        return $employees;
    }
    ...
    {
        $stmt = $this->connection->prepare("SELECT * FROM `employees` WHERE first_name LIKE ? OR last_name LIKE ? OR id = ? OR CONCAT(LOWER(first_name), ' ', LOWER(last_name)) LIKE ? OR location LIKE ? ORDER BY location");
        $like = "%$query%";
    ...
    public function import(string $type, mixed $file): array
    {
        ...
        $count = 0;
        foreach ($rows as $row) {
            $employee = new Employee($row[0], $row[1], $row[2], $row[3]);
            $this->add($employee);
            $count++;
        }
