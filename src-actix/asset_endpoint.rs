...
pub async fn index(_req: HttpRequest) -> Result<impl Responder, Error> {
...
pub async fn proxy_to_vite(req: HttpRequest, mut payload: web::Payload) -> Result<HttpResponse, Error> {
...
pub fn start_vite_server() -> Result<Child, std::io::Error> {
	...
	let vite = String::from_utf8(vite).map_err(|_| {
		std::io::Error::new(
	...
	std::process::Command::new(vite)
		...
		.spawn()
		.map_err(|err| {
			error!("Failed to start vite server: {}", err);
			err
		})
}
